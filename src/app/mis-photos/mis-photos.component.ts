...
@Component({
  selector: 'app-mis-photos',
  templateUrl: './mis-photos.component.html',
  styleUrls: ['./mis-photos.component.scss'],
  standalone:true,
  imports: [
    CommonModule,
    NgForOf,
...
export class MisPhotosComponent  implements OnInit {
...
constructor(private sanitizer: DomSanitizer, private auth: FirebaseService, private router: Router, private photosService: PhotosService,
  ...
  async ngOnInit() {
    ...
    this.usuarioActual = this.auth.usuarioAutenticado.email;
    ...
    console.log(this.usuarioActual);
    this.photos = photos.filter((photo: { currentUser: string; }) => photo.currentUser === this.usuarioActual);
    ...
    const fotosConLikes =this.photos;
    const data = {
  ...
  tomarFotos() {
    this.photosService.takePhoto("cosasFeas").then((photos) => {
